...
$body = elgg_echo('elgg_update_services:next_check') . ' ' . date('Y/m/d H:i', elgg_get_plugin_setting('execution_date', 'elgg_update_services')) . "<br><br>";

if (is_array($update_result["result"]) && (count($update_result["result"])) > 0) {
	foreach($update_result["result"] as $result) {
		$plugin_detail = elgg_view('elgg_update_services/update_detail', ['elgg_update_services_update_detail' => $result]);
